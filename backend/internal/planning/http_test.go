...
func TestHTTPCreateUsesScopedProject(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/v1/sprints", strings.NewReader(`{"project_id":"project-2","name":"Wrong project"}`))
	...
	request = httptest.NewRequest(http.MethodGet, "/api/v1/sprints", nil)
	request.Header.Set("X-Organization-ID", "org-1")
	request.Header.Set("X-Actor-ID", "user-1")
	response = httptest.NewRecorder()
	handler.ServeHTTP(response, request)
